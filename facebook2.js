		...
		getRecognitionMetadata: function(imgId, callback, _ct) {
			...
			setTimeout(function() {
				THIS.httprequest.post({
					...
					headers: headers,
					body: 'recognition_project=composer_facerec&photos[0]=' + imgId + '&target&is_page=false&include_unrecognized_faceboxes=true&include_face_crop_src=true&include_recognized_user_profile_picture=true&include_low_confidence_recognitions=true&' + req_parms,
					gzip: true
				...
				THIS.httprequest.get({
					...
					THIS.graph.post('/me/photos', params, function(err, r) {
						...
						THIS.getRecognitionMetadata(imgId, function(result) {
							if (!result || !result.faceboxes) {
							...
							} else if (result.faceboxes[0].recognitions.length == 0) {
								_callback({
									error: {"message": 'Facebook couldn\'t recognize this picture.',
									code: -502}
								});
							} else if (result.faceboxes[0].recognitions[0].certainty < 0.85) {
								_callback({
								...
								var mdata = {
									certainty: result.faceboxes[0].recognitions[0].certainty,
									name: result.faceboxes[0].recognitions[0].user.name,
									fbid: result.faceboxes[0].recognitions[0].user.fbid,
									profile_picture: result.faceboxes[0].recognitions[0].user.profile_picture
								};
